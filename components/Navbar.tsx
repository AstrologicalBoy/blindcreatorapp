  ...
  return (
    <nav className="relative z-10 bg-lightScreenBg dark:bg-darkScreenBg shadow-md dark:shadow-darkKey2Bg/20 transition-all duration-300">
      <div className="flex flex-col gap-3 md:flex-row justify-between items-center p-4">
        <a href="/products"  onClick={() => setActualPage(1)} className="font-bold text-3xl md:text-base text-lightTextColor1 dark:text-darkTextColor1">Blind Creator</a>
        <div className="flex flex-col md:flex-row items-center justify-center">
          <Link href="/products/favorites" className="text-center md:text-left md:mr-4 text-lightTextColor1 hover:text-black dark:text-darkKey2Shadow dark:hover:text-darkKey2Bg transition-colors duration-200">Favoritos</Link>
          ...
          <button onClick={() => makeLogout.mutate()} title="Cerrar sesión" className="font-bold px-4 py-2 mt-5 md:mt-0 text-white bg-red-600 hover:bg-red-800 transition-colors duration-200 rounded-md"><LogOut /></button>
        </div>
      </div>
      <CartDrawer />
    </nav>
